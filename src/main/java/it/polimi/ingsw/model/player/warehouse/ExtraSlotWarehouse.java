...
public class ExtraSlotWarehouse extends WarehouseDecorator{

    @Expose
    private final Shelf extraShelf;

    public ExtraSlotWarehouse(Warehouse warehouseToBeDecorated, ResType theme ){
        super(warehouseToBeDecorated);
        this.extraShelf = new Shelf(2, true, theme);
    }

    /**
     * Gives the shelf. If i=3 it returns the extraShelf provided by the active LeaderCard
    ...
    /**
     * Allow to insert a resource in a shelf, or if i=3 in the extra one
     ...
     * which represent all the resources that I have between the shelves and the strongbox, now also in the extra shelf
    ...
    public boolean checkAvailabilityResources(ArrayList<Resources> wantToUse){
        int check = 0;
        ArrayList<Resources> allResources = this.allResources(); //this is the reason of overriding this method, because it exploits this.allResources()
        int indexOfResource;
     ...
     * Decrease the amount of resources, first, from the extra shelf, second from the shelves and then,if remaining, from strongbox
     ...
     * In last positions are added those which belongs to the extra shelf
    ...
    public ArrayList<Resources> possibleResources(){
        ...
        for(int i = 0; i<2- extraShelf.getShelfNumberOfResources(); i++) possibles.add(new Resources(extraShelf.getShelfCurrentType(), i+1));
        return possibles;
    }

    /**
     * This method is used in checkAvailabilityResources!
     * Return a list of all the resources that I have first in the strongbox, then "extra shelf" and finally the shelves, all the resources appear in the list one time and the counter is incremented
     ...
     * Return a list of all the shelves placing the one provided by the leaderCard in the first position
